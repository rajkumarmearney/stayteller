...
namespace Modules\Agencies\Admin;
...
class AgenciesController extends AdminController
{
    ...
    public function store(Request $request, $id)
    {
        ...
        if (!empty($request->get('list_agent'))) {
            ...
            if (!$check)
                return redirect()->back()->with('error', __('Agent only belong one agencies'));
        }
    ...
    public function bulkEdit(Request $request)
    {
        ...
        try {
            switch ($action) {
                case "delete":
                    $collection->delete();
                    DB::table('agencies_agent')->whereIn('agencies_id', $ids)->delete();
