...
class NewsController extends FrontendController
{
    ...
    public function index(Request $request)
    {
        ...
        $data = [
            ...
            'featured_news'     => News::query()->with('translations')->where("status", "publish")->where('is_featured', 1)->limit(3)->get(),
            'breadcrumbs'       => [
                [
                    'name'  => __('News'),
    ...
    public function detail(Request $request, $slug)
    {
        ...
        //update news count
        News::where('slug', $slug)->where('status','publish')->update(['view' => $row->view + 1]);
        ...
        $data = [
            ...
            'featured_news'     => News::query()->with('translations')->where("status", "publish")->where('is_featured', 1)->limit(3)->get(),
            'related_news'      => News::query()->with('translations')->where('status','publish')->where('cat_id', $row->cat_id)->where('id', '!=', $row->id)->limit(2)->get(),
            'custom_title_page' => $title_page ?? "",
            'breadcrumbs'       => [
                [
                    'name' => __('News'),
